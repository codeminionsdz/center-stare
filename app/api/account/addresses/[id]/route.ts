...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.log("Available address IDs:", userAddresses?.map((a: any) => a.id) || [])
    ...
    if (checkError || !existingAddress) {
      ...
      console.error("User addresses available:", userAddresses?.map((a: any) => a.id))
      return NextResponse.json(
        {
          success: false,
          ...
          details: {
            ...
            userId: user.id,
            availableAddresses: userAddresses?.map((a: any) => ({ id: a.id, user_id: a.user_id })) || [],
    ...
    if (existingAddress?.user_id !== user.id) {
      ...
      return NextResponse.json({ success: false, error: "Not authorized to update this address" }, { status: 403 })
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ success: false, error: updateError.message || "Failed to update address" }, { status: 500 })
    }
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // Verify the address belongs to the user
    const { data: existingAddress } = await supabase.from("addresses").select("user_id").eq("id", addressId).single()
    ...
    // Delete the address
    const { error: deleteError } = await supabase.from("addresses").delete().eq("id", addressId)

    if (deleteError) {
      console.error("Error deleting address:", deleteError)
      return NextResponse.json({ error: "Failed to delete address", details: deleteError.message }, { status: 500 })
    }
